...
def visualize(epoch_id, machine_steering, out_dir, perform_smoothing=False,
    ...
    vid_mp4_path = join_dir(epoch_dir, 'epoch{:0>2}_front.mp4'.format(epoch_id))
    ...
    import imageio
    out_path_mp4 = join_dir(out_dir, 'epoch{:0>2}_human_machine.mp4'.format(epoch_id))
    ...
    writer.close()
    # mkv_to_mp4(out_path, remove_mkv=False)
